...
> **⚠️ 2026-08-02 갱신 (SYNC.md §2 N8) — 코어 스왑 완료.** 아래 §0/§1의 "`?real=1`"·"기본값=Mock"
> 서술은 전부 스왑 **이전** 시점 기록이다. 지금은 **기본값 = 실제 `GameCore.js`**이고, `?real=1`은
> 폐기됐다 — 전체 루프가 자동으로 도는 Mock 데모를 보고 싶으면 대신 `?mockcore=1`을 쓴다
> (`?mock=1`은 `main.js` 라우터가 이미 별도 `MockScene` 진단 화면으로 선점하고 있어 이름이 다르다).
> 화면 배지도 "REAL CORE"(상시 표시)가 아니라 "MOCK DEMO"(`?mockcore=1`일 때만)로 바뀌었다.
> 아래 본문은 스왑에 이르기까지의 판단 과정 기록이라 그대로 남겨둔다.
...
| 플래그 | 씬 | 용도 | 소유 |
|---|---|---|---|
| (기본, 없음) | `GameScene` | ✅ **2026-08-02부터 실제 게임 화면(`GameCore.js`로 동작)** — 스왑 이전엔 `MockGameCore`였다 | 공용 |
...
| `?mockcore=1` | `GameScene` 내부 | ✅ **2026-08-02, `?real=1` 대체.** 기본값(실제 코어) 대신 전체 루프가 자동으로 도는 `MockGameCore` 데모로 붙는다. 화면 좌상단에 "MOCK DEMO" 배지가 뜬다. QA/FX 검증용 | 공용 |

`?fxtest=1`과 `?debug=1`은 숫자 키가 겹치면 서로 무력화되므로 **완전히 분리된 플래그**다. 서로 조합해서 켜도 무방(각 씬이 무관한 플래그는 무시). `?mockcore=1`은 숫자 키를 안 쓰므로 `?debug=1`과 같이 켜도 안전하다(`?mockcore=1&debug=1`).
