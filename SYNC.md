...
### [기록] N9 · B · 2026-08-02 — HANDOFF.md 갱신 + 세운상가 오라 원 depth 버그 + 배경 낮/밤을 웨이브 주기로 변경

**한 일 3건 (전부 사용자 요청, 즉시 반영):**
1. **`HANDOFF.md` 갱신** — N8 코어 스왑 이후에도 `?real=1`/"기본값=Mock"으로 남아있던 §0/§1 서술이
   당장 봐도 틀린 상태라 A가 인계 문서 보고 헷갈릴 수 있었다. 전체를 다시 쓰진 않고(과거 판단
   과정 기록이라 그대로 둠) 상단에 "2026-08-02 갱신" 경고 블록 + §1 플래그 표의 `?real=1` 행만
   `?mockcore=1`로 교체
2. **세운상가(오라형 서포터) 배치 시 분홍 오라 원이 드래프트 카드 위로 겹쳐 보이던 버그** —
   `BuildUI.js`의 `auraGfx`가 `setDepth(49)`인데 `DraftOverlay.js`는 어떤 요소도 `setDepth()`를
   안 불러서 기본값(0)으로 그 아래 깔려 있었다. `DraftOverlay.js`에 `OVERLAY_DEPTH=500` 상수를
   추가해서 dim/배너/카드 전부에 적용 — BuildUI·UpgradeUI의 사거리·오라 원(45~62)보다는 항상
   위, BossAlert(9000대)·GameOverScene(9999)보다는 아래로 둬서 그쪽 화면 전환이 항상 최우선이
   되게 함. 실제로 세운상가를 지어보고 레벨업 드래프트를 다시 띄워서 카드 아래로 오라 원 테두리
   일부만 살짝 비치고 카드 자체는 안 가려지는 것까지 확인함
3. **배경 낮/밤 전환을 접속 시각 연동에서 5웨이브 주기로 변경**(합의 사항) — `mapView.js`의
   `resolveBgKey()`(시각 기반)를 `bgKeyForWave(wave)`로 교체. wave 1~5=낮, 6~10=밤, 11~15=낮 ...
   (wave 0은 1로 취급). `EV.waveStarted`를 구독해서 텍스처만 교체(위치 유지). `?bg=day|night`
   강제 지정은 그대로 유지(촬영용 — 지정하면 웨이브 이벤트 구독 자체를 안 함). `?debug=1`의
   웨이브 점프 키(1~9)로 직접 확인 — wave 1=낮, wave 6=낮→밤 전환, wave 1로 복귀 시 다시 낮
   전환까지 정상 동작 확인. `UITheme.js`/`CLAUDE.md` 관련 주석·스코프 문구도 같이 갱신
...
| D23 | 2026-08-02 | **"4단계 강화 = 복제" 시스템(CLAUDE.md §5-7) — 유니크 룰의 유일한 예외.** 최대 레벨 건물을 골드로 "복제"하면 그 종류의 추가 설치권(`GameCore` 내부 `clonePicksById: Map<id,count>`, kind 구분 없이 id 하나로 충분 — 타워·서포터 id가 안 겹침)을 얻는다. `Tower`/`Supporter` 생성자에 `instanceIndex`(같은 id로 먼저 지어진 개수, 0부터, 건설 시점에 고정) 추가 — `upgradeCost()`가 `waves.json`의 신규 `cloneScale.upgradeCostMulPerInstance^instanceIndex`를 곱한다(instanceIndex=0이면 기존 공식과 100% 동일). `GameCore.cloneCost/clone` 신규, `EV.cloneAcquired {kind,id}` 신규, `REJECT.notMaxLevel` 신규. `canBuildTower/canBuildSupport`의 unique 판정은 "이미 있어도 그 id의 복제 설치권이 있으면 통과"로 예외 처리. `BuildUI.js`는 `clonePicks` 맵으로 이미 지은 타워/서포터 슬롯을 복제권이 남은 동안 다시 선택 가능하게 하고(✓ 대신 x{n} 뱃지), `UpgradeUI.js`는 최대 레벨 패널에서 "최대 강화 완료" 대신 복제 비용+버튼을 보여준다(계속 눌러서 여러 개 쌓을 수 있음, 비용은 그때마다 재계산). N서울타워는 복제·재배치 둘 다 제외(상시 지형물, TowerView 실체 없음) | 사용자가 유니크 룰(CLAUDE.md §1 절대 규칙2)을 의도적으로 완화하는 로그라이트 파워 스파이크 메커니즘으로 요청. 비용 배율을 타워/서포터 8종 각각에 두지 않고 `waves.json`에 전역 하나로 둔 것은 A의 설계 판단(사용자가 "towers.json 또는 waves.json 중 네가 판단" 위임) — P4에서 조정 지점을 하나로 모으기 위함 |
